...
function joinPlayingGroup(player, values) {
    // let's find the first playing group
    for (var i = 0; i < player.discovery.zones.length; i++) {
        var zone = player.discovery.zones[i];
        if (zone.coordinator.state.currentState == 'PLAYING') {
            attachTo(player, zone.coordinator);
            break;
        }
    }
}
...
module.exports = function (api) {
  ...
  api.registerAction('joinplaying', joinPlayingGroup);
}
