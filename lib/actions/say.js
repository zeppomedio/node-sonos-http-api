...
function sayAll(player, values, callback) {
  ...
  */
  ...
  // ~mikeyk HACK to play local file
  preset.uri = 'https://s3.amazonaws.com/zeppomedioalexa/chime.m4a';
  player.discovery.applyPreset(preset, function () { if (preset.state == 'stopped') startPlayingWhenOneBigGroup = true});
  coordinator.on('transport-state', transportStateClosure(coordinator, backupPresets));
  callback();
}
